...
原有 12 张 fixture 全是照片，风格化输入（插画/线稿/像素/贴纸）是盲区。补 5 张程序化生成的
风格图（`Scripts/make-styled-fixtures.swift`，来源零外部依赖），17 图 × 3 引擎全矩阵跑
CLI（51 次推理，49 成功）。中位耗时：Vision 366ms、lite/matting ≈1.7s（CLI 冷启动含模型加载）。

**逐风格结论**：

| 输入 | Vision | BiRefNet_lite | lite-matting |
|---|---|---|---|
| 扁平插画（illo-01/02） | ✅ 最干净 | ⚠️ illo-02 把背景楼房抠了一部分进来 | ⚠️ illo-01 把整条山丘保留了 |
| 线稿（lineart-01） | ❌ **翻车**：丢屋顶、只留半座房 | ✅ 完整 | ✅ 完整（线条略淡） |
| 像素（pixel-01） | ✅ 硬边保持 | ✅ 基本干净 | ❌ 边缘糊化、残底（羽化天性和像素风相克） |
| 贴纸（sticker-01） | ✅ | ✅ | ✅ |
| 玻璃（glass-01，照片） | 不透明处理 | 不透明处理 | ✅ **真透明**（玻璃透出棋盘格——独有能力） |
| 无主体（nosubject-01/02） | ✅ **正确拒绝**（exit 2） | ❌ 硬抠出护栏碎片 | ❌ 硬抠出树枝碎片 |

**审计结论**：

1. **1.0 不引入新模型**。三引擎互补性成立且各有不可替代点：Vision 快、对多数风格化输入
   意外地稳、且是唯一会对无主体图诚实说"不"的引擎；lite 补上 Vision 的线稿短板；matting
   的真透明输出（玻璃）是独有能力。"不同图换不同引擎"这个核心卖点被实测坐实——线稿一行
   就是给用户看的换引擎理由。
2. **已知弱点（记 backlog，不阻塞发布）**：
   - BiRefNet 系对无主体图不会拒绝（DIS 模型天性，总要输出个 mask）。可行的优化：引擎层
     对 mask 覆盖率过低/过碎的结果判 `noSubjectDetected`——有误杀风险，需要阈值实验。
   - matting 对硬边风格化图过度羽化——与"柔细边"的产品定位一致，文案已经在管理预期。
3. **仍存的盲区**：真正的日式动漫画风（anime character）没有可再分发的测试图，程序化画
   不出来。候选补法：ImageGen 生成（同图标产线）。若实测拉胯，引入候选是
   **SkyTNT/anime-segmentation**（Apache-2.0，需自转 Core ML，走 §4.8 同一条管线）——
   manifest 加一条记录即可，架构无改动。
