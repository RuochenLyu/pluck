...
- **菜单栏常驻**：图标即拖放目标；点击弹出 popover（整体可拖放 + Recent 网格）。剪贴板流程：popover 内 ⌘V → 抠图 → 结果自动写回剪贴板 → 目标 app ⌘V，**中间不落盘**（2026-07-27 起废弃全局快捷键方案，见 decisions.md）。点击 Recent 项打开预览面板：before/after 拖拽滑块 + Copy/Save。
