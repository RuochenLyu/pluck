...
- ~~**图标**：实心有机 blob + 虚线剪影（主体被拽走留下轮廓）；18px 菜单栏版虚线简化为 4–5 段粗 dash，进开发后直接画矢量模板 PDF。羽毛方案废弃。~~ **2026-08-14 定稿替换**（见 decisions.md 同日）：图标使用 `Packaging/AppIcon.icon` 的珊瑚色照片吉祥物，`bundle.sh` 通过 `actool` 编译为 `AppIcon.icns` + `Assets.car`。菜单栏图标随 shelf 删除，不再需要。
