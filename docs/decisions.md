...
## 2026-07-28 — 网络例外之二：Sparkle 更新检查（默认开，可关，日检）

- **背景**：CLAUDE.md 规定任何新增网络请求是重大决策。Sparkle 是 app 第二个也是最后一个网络行为（第一个是用户显式触发的模型下载）。
- **决策**：默认开启自动检查，频率每日一次；Settings 显式开关（关闭即零请求）；appcast feed 与更新包放本仓库 GitHub Releases（不引入第三方服务器），更新包 EdDSA 签名，公钥编进 app——签名链与 §4.8 模型校验同构：仓库签名背书 feed，EdDSA 背书字节。
...
## 2026-07-29 — Settings 模型区：默认引擎是一个 Picker，行退化为管理清单；缓存体积上按钮

- **决策**：模型节首行是 `New images use / 新图片使用` 的 Picker（只列 Vision + 已安装），下方行**删掉选中圆圈**，退化为纯管理清单——图标砖 + 人话名 + 一句 blurb + 一行小字技术信息 + 右侧 Download/Cancel/Delete，五样，排版呼吸感对齐 History 节。Vision **不再单独占一行**。**这条推翻 2026-07-28「Settings 的 Models 一节保留列表、换说法」里"选择即是行（p5）"那半条**。
- **为什么推翻**："选择即是行"在两行的时候读得通，一旦一行正在下载、下载失败、或者根本没装就崩了：一个因为字节还没到而置灰的圆圈，是在用**偏好设置**的词汇回答一个关于**下载**的问题。两个问题（新图用哪个引擎 / 这台机器上有什么）应该分开问。Vision 那一行的存在理由本来就是"它是可选项之一"，选择挪进 Picker 之后它在一个主题是"磁盘上的字节"的清单里没有 Download 也没有 Delete，就只剩家具。
- **Picker 的 binding 读时回落、不写回**：`Preferences.engineID` 故意保留指向已删除模型的 id（2026-07-28 那条：会自己偷偷改写的偏好更糟），所以 Picker 显示的是**今天这次拖入实际会用的那个**，只有用户主动选择才落盘。
- **体积一律用实际磁盘，不用 manifest**：`descriptor.bytes` 是**zip 的长度**——那是 Download 即将花掉的**带宽**，不是 Delete 即将还回来的**磁盘**。装好之后行里的数字改为 `DirectorySize.bytes(of:)` 实测解压后的 `.mlpackage`；没装时仍报 manifest 数字，因为那时这一行提供的就是一次下载。删除时把测量值一并丢掉，否则行会说出一个它已经没有的模型占了多少磁盘。
- **History 的 Clear 按钮带体积后缀**：`Clear recent cutouts (412 MB)`。一个没有数字的清空按钮没人敢按，因为没人知道值不值。**遍历而不是维护累加器**：一个跑在写入/提升/淘汰/回收站/启动清扫每条路径上的运行总数，第一次漂移就是在一个卖点是"对字节诚实"的 app 里对磁盘占用说了一句自信的假话。遍历几百个文件是毫秒级的，而且永远是真的。四舍五入到 0 时不显示——"(0 MB)" 是一个按钮在用警告的形状宣布按了它什么也不会发生。
- **"可从废纸篓找回"这句只对开着历史时说**：`CutoutArchive.discard` 对 history 根走 `trashItem`，对 session 根直接删——后者是对的，用户自己说了不要留。一句不分情况的"你可以找回来"是 session 那半边兑现不了的承诺。

## 2026-07-29 — 截图验收改为进程内渲染：`ImageRenderer` 遇到 Liquid Glass 一律全透明

- **背景**：本机 `screencapture` 因缺屏幕录制权限一律返回全黑，而这个仓库的验收惯例是"逐面截图核对"（2026-07-28 那条引擎 chip 的 bug 就是纯读代码看不出来的）。
...
## 2026-08-11 — 图标定稿：cat-1（猫走出照片卡），ImageGen 分层素材 + 脚本合成

- **过程**：程序化贝塞尔三轮验证构图（果实/拨片/对分/拎出）后确认插画质感非代码可达，改走"任务书 → ImageGen 概念 18 张 → 人筛（32px 可读 + 叙事测试）→ 分层素材重生成"的产线（docs/prototypes/icon-brief.md 附一/附二，icon-concepts/ 与 icon-composer/ 是两轮产物）。选中 cat-1：珊瑚猫走出微斜照片卡，卡上留猫形洞，黄昏山景说明"这是照片"。
